...
/**  let spaceViewer = new D3D.D3DSpaceViewer({
      ...
      linkText: 'View in 3D',
      previewCtrCls: 'collection-wrapper', //container element in which to create the preview
      skyboxes: true
...
function postToLines(posts){
  let fullText = posts.PostsFound[postIndex].Body;
  let words = fullText.split(' ');
  let lines = splitArrayIntoChunksOfLen(words, 2);
  return lines;
}
...
function splitArrayIntoChunksOfLen(arr, len) {
  ...
  function init(el, posts) {
      ...
      lines = postToLines(posts);
      lineIdx = 0;
      setInterval(function(){
        if(lines[lineIdx]){
          text = lines[lineIdx].join(' ');
          refreshText();
          ++lineIdx;
          if(targetRotation=== 0.5){
            targetRotation = -6;
          } else {
            targetRotation = 0.5;
          }
        ...
        document.body.addEventListener( 'keypress', onDocumentKeyPress );
...
*/
      ...
      function loadFont() {
        ...
        loader.load( 'fonts/MODERN_WARFARE_Regular.json', function ( response ) {
